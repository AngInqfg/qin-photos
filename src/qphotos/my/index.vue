<template>
  <Layout ref="LayoutRef" class="h-screen"
...
import { queryMyList } from "@/https";
import type { ImageItem } from "@/types/index";
const LayoutRef = ref();
const cTypeRef = ref();
const scrollTop = ref(0);
const params = ref<myParamsDTO>({
  pageNum: 1,
  pageSize: 30,
  total: 0,
  search: "",
...
function getList() {
  queryMyList(params.value).then((res) => {
...
const data = computed<ImageItem[]>(() =>
  useDataStore()?.myData?.length > 0 ? useDataStore().myData : [],
);
watchEffect(() => {
